...
function DisplayPersonalityData_Ctrl (personality_MO) {
...
function _displayResultsNow() {
  ...
  setTimeout(function(){
    ...
    // $('html, body').animate({
    //     scrollTop: $('.personality-result').offset().top - 20
    // }, 1000);
...
function _loadPersonalityBrands(){

	$('.barchart-graph').show();

    var dataProvider = [];
    $.each(this.personality_MO.brands_matched, function( index, value ) {
        dataProvider.push({
    ...
    var chart = AmCharts.makeChart( "chartdivBarChart", {
      "type": "serial",
      "theme": "none",
      "dataProvider": dataProvider,
      "valueAxes": [ {
...
function _loadConsumptionPreferences () {
  ...
  filteredConsumptionsArray.forEach(function(consumpGroup,index) {
    var groupPreference = $(`<div class="col-md-12">      
                    <div class="mt-3">
                        <div class="mb-3" style="font-weight: bold;font-size:18px;"><span class="circle-number`+_getCircleColor()+`">`+(index+1)+`</span> `+consumpGroup[0].groupName+`</div>
...
module.exports = DisplayPersonalityData_Ctrl;