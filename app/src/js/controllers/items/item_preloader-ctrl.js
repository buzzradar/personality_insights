...
function _prepareAPICall() {

    var dataSource = DisplayGlobals_SRV.getPagesArray()[0].pagesArray[0].pageDataMariusObj.source;  //Influencer or Brand    

    if (dataSource == "influencer") {
        var objToSend = {
            twitter_username : DisplayGlobals_SRV.getPagesArray()[1].pagesArray[0].pageDataMariusObj.your_twitter_handle,
        };
        _makeAPICall.call(this, objToSend);
    }else{
        //Brand, so we use the hardcoded json objects that you can find in item_brandslisst
        var brandJson = DisplayGlobals_SRV.getPagesArray()[1].pagesArray[0].pageDataMariusObj.brandJsonObj;
        var brandName = DisplayGlobals_SRV.getPagesArray()[1].pagesArray[0].pageDataMariusObj.brand;

        DisplayGlobals_SRV.setPersonalityInsightsData({type:"brand",data: brandJson, brand_name: brandName});
...
function _makeAPICall(objToSend) {

    var _this = this;

    console.log("Twitter handle--->", objToSend);

    $.ajax({
        type: 'POST',
        url: 'https://sockets.buzzradar.com:3000/watson-pi-demo',
        async: true, 
        ...
        success: function(json) {
          ...
          if (json.error) {
            ...
            //All good carry on
            DisplayGlobals_SRV.setPersonalityInsightsData({type:"influencer",data: json});
            _updateMailingList.call(this);
            _showCongratulations.call(this);
...
function _updateMailingList() {
    ...
    console.log(DisplayGlobals_SRV.getPagesArray());


    var keepInformed = DisplayGlobals_SRV.getPagesArray()[2].pagesArray[3].pageDataMariusObj.keep_informed

    if (keepInformed) {

        var mailChimpObj = {
            "mc_signupsource" : 'hosted',
            "EMAIL" : DisplayGlobals_SRV.getPagesArray()[2].pagesArray[3].pageDataMariusObj.your_email,
            "FNAME" : DisplayGlobals_SRV.getPagesArray()[2].pagesArray[0].pageDataMariusObj.first_name,
            "LNAME" : DisplayGlobals_SRV.getPagesArray()[2].pagesArray[1].pageDataMariusObj.last_name,
        };
