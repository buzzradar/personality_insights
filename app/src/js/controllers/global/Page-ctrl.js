...
const ITEM_2choices = require('../items/item_2choices-ctrl');
const ITEM_brandslist = require('../items/item_brandslist-ctrl');
...
function _loadIntroText() {
    ...
    var newIntroTextWithRoot = this.page_MO.pageConfigObj.introtext.replace('"images/','"'+DisplayGlobals_SRV.getArguments().root+'images/');
    this.PAGE_INTRO.html(newIntroTextWithRoot);
...
function _loadCTAButtons() {
    ...
    if (_isEmpty(this.page_MO.pageConfigObj.cta)) {
        $('.form-actions').css('border-top','0px');
    }else{
        $('.form-actions').css('border-top','1px solid #eee');
    }
